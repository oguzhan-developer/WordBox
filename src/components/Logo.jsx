...
export function Logo({ withText = true, size = 'md', to = '/dashboard' }) {
    ...
    return (
        <Link to={to} className="flex items-center gap-3 group">
            <div className={`relative ${sizeClass} rounded-2xl overflow-hidden shadow-[0_10px_30px_-8px_rgba(0,0,0,0.35)] bg-gradient-to-br from-amber-300 via-orange-400 to-pink-500 text-black transition-transform group-hover:-translate-y-0.5`}>
                <div className="absolute inset-0 bg-white/20 blur-md" aria-hidden="true" />
                <div className="absolute inset-[4px] rounded-xl bg-gradient-to-br from-white/80 via-amber-50 to-white/50 opacity-70" aria-hidden="true" />
