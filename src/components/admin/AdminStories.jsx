...
import { Search, Plus, Save, Trash2, Edit, Eye, EyeOff, RefreshCw, ChevronDown, ChevronUp, BookOpen } from 'lucide-react';
...
export default function AdminStories() {
    ...
    const handleEdit = async (item) => {
        ...
        if (item.story_levels) {
            await Promise.all(item.story_levels.map(async (level) => {
        ...
        setFormData({
            ...
            category_id: item.story_categories?.id || item.category_id || '',
            levels: parsedLevels
        });
            ...
            for (const [level, data] of enabledLevels) {
                ...
                if (data.selectedWords && data.selectedWords.length > 0) {
                    const wordInserts = data.selectedWords.map((word, index) => ({
                        story_level_id: insertedLevel.id,
                        word_id: word.id,
                        display_order: index,
    ...
    return (
        <>
            ...
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Hikayeler</h2>
                <button
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    {filteredItems.map(item => {
                        const levelsData = item.story_levels;
                        const categoryData = item.story_categories;
            ...
            <Modal
                ...
                title={`${showEditModal ? 'Düzenle' : 'Yeni Ekle'}: Hikaye`}
                size="xl"
            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="hikaye-basligi-slug"
