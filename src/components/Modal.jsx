...
export default function Modal({
    ...
    return (
        ...
        >
          ...
          animate-slideUp max-h-[90vh] overflow-y-auto border border-white/20 dark:border-dark-border
          ${className}
        `}
            >
                {/* Header */}
                {(title || showCloseButton) && (
                    <div className="flex items-center justify-between p-6 border-b border-gray-200/50 dark:border-dark-border sticky top-0 glass-strong backdrop-blur-xl z-10 rounded-t-3xl">
                        {title && (
                            <h2
                        ...
                        {showCloseButton && (
                            <button
                                onClick={onClose}
                                className="ml-auto size-10 flex items-center justify-center rounded-xl glass hover:bg-danger-500/10 transition-all hover:scale-110"
                                aria-label="Kapat"
                            >
...
// Confirmation Modal with consistent colors
        ...
        <Modal isOpen={isOpen} onClose={onClose} size="sm" showCloseButton={false}>
            <div className="text-center">
                ...
                <div className="flex gap-3 justify-center">
                    <button
                        onClick={onClose}
                        className="px-4 py-2 rounded-lg border border-gray-300 dark:border-dark-border text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                    >
                        {cancelText}
                    </button>
                    <button
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="sm" showCloseButton={false}>
            <div className="text-center">
                ...
                {/* XP Display */}
                {xpEarned && (
                    <div className="inline-flex items-center gap-2 px-4 py-2 gradient-accent text-white rounded-full font-bold mb-4 shadow-lg shadow-accent-500/30">
                        <span>+{xpEarned}</span>
                        <span>XP</span>
                    </div>
                )}
                ...
                {badge && (
                    <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-xl mb-4">
                        ...
                        <p className="font-medium text-gray-900 dark:text-dark-text">{badge.name}</p>
